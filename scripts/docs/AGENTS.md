...
- Renders all chapters into a single PDF via Quarto + TinyTeX (LuaLaTeX)
...
- **`pdf.ts`** — Bun script that:
  - Runs `quarto render --to pdf` (TinyTeX auto-installs missing LaTeX packages)
...
- Don't run `quarto render` without TinyTeX installed (`quarto install tinytex`)
...
These are mentioned in `lib/mermaid-rules.ts` via `RULE_ALTERNATIVE_DIAGRAM_ENGINES`.

## Arabic Support

- Arabic on the cover page uses `polyglossia` with `\setotherlanguage{arabic}` and Cairo font
- Arabic text must be wrapped in `\begin{Arabic}...\end{Arabic}` blocks in LaTeX
- **Never load `bidi` package directly** — it conflicts with LuaLaTeX. Polyglossia auto-loads `luabidi` instead.
